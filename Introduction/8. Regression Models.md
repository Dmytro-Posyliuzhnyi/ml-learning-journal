...
In standard linear regression, we minimize the following loss function:
...
Ridge Regression adds a penalty term to this loss function to shrink the weights:
$`Loss = \frac{1}{n} \sum_{i=1}^n \left( y_i - \hat{y}_i \right)^2 + \lambda \sum w_j^2`$
...
In standard linear regression, the model minimizes the following loss function:
$`Loss = \frac{1}{n} \sum_{i=1}^n \left( y_i - \hat{y}_i \right)^2`$

Lasso Regression adds an **L1 penalty** to this loss:
$`Loss = \frac{1}{n} \sum_{i=1}^n \left( y_i - \hat{y}_i \right)^2 + \lambda \sum |w_i|`$
...
$`Loss = \frac{1}{n} \sum_{i=1}^n \left( y_i - \hat{y}_i \right)^2 + \lambda \cdot \left( \alpha \sum |w_i| + (1 - \alpha) \sum w_i^2 \right)`$
